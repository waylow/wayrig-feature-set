...
class Rig(BasicChainRig):
    ...
    ####################################################
    # TONGUE CONTROL EDITS

    #Add Tongue Master
    @stage.generate_bones
    def generate_tongue_master(self):
        if self.params.add_tongue_master:
            master = self.copy_bone(self.bones.org[0], 'Tongue_master', parent=True)
            align_bone_to_axis(self.obj, master, 'y')



    #World Align Controls
    @stage.parent_bones
    def world_align_tongue_bones(self):
        if self.params.world_align_controls:
            orgs = self.bones.org
            for org in orgs:
                align_bone_to_axis(self.obj, strip_org(org), 'y')
            align_bone_to_axis(self.obj, strip_org(orgs[-1]) + '_end', 'y')


    @stage.apply_bones
    def parent_tongue_bones(self):
        if self.params.add_tongue_master:
            orgs = self.bones.org
            # controls Start/End
            self.set_bone_parent(strip_org(orgs[0]), 'Tongue_master', use_connect=False, inherit_scale='FULL')
            self.set_bone_parent(strip_org(orgs[-1]) + '_end', 'Tongue_master', use_connect=False, inherit_scale='FULL')
            # org and DEF
            self.set_bone_parent(orgs[0], 'Tongue_master', use_connect=False, inherit_scale='FULL')
            self.set_bone_parent(self.bones.deform[0], 'Tongue_master', use_connect=False, inherit_scale='FULL')
            # handles
            for org in orgs:
                self.set_bone_parent(make_derived_name(org, 'mch', '_handle'), 'Tongue_master', use_connect=False, inherit_scale='FULL')
            self.set_bone_parent(make_derived_name(org, 'mch', '_end_handle'), 'Tongue_master', use_connect=False, inherit_scale='FULL')
            # MCH _poffset
            ignore = [0, -1]
    ...
    def add_parameters(self, params):
        params.world_align_controls = bpy.props.BoolProperty(
    ...
    def parameters_ui(self, layout, params):
        layout.prop(params, "world_align_controls")
        layout.prop(params, "add_tongue_master")
