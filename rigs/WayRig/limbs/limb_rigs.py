...
from rigify.utils.bones import BoneDict, put_bone, align_bone_orientation, set_bone_widget_transform
...
class BaseLimbRig(BaseRig):
    ...
    def configure_master_control(self):
        ...
        if self.params.make_preserve_volume:
            text = 'Preserve Volume ('+ self.bones.ctrl.master + ')'
            self.make_property(self.bones.ctrl.master, 'volume_preserve', 0.0, description='Preserve volume in stretch')
            panel.custom_prop(self.bones.ctrl.master, 'volume_preserve', text=text, slider=True)
    ...
    def make_fk_control_widget(self, i, ctrl):
        if i < 2:
            create_circle_widget(self.obj, ctrl, radius=0.25, head_tail=0.5)
        elif i == 2:
    ...
    def configure_ik_controls(self):
        ...
        ik_pole = self.get_bone(self.bones.ctrl.ik_pole)
        ik_pole.rotation_mode = 'XYZ'
        ik_pole.lock_rotation = True, True, True
        ik_pole.lock_scale = True, True, True
    ...
    def make_tweak_bone(self, i, entry):
        ...
        name = make_derived_name(entry.org, 'ctrl', suffix)
    ...
    def make_deform_bone(self, i, entry):
        ...
        if self.params.make_bendable_foot and self.segments > 1:
            self.get_bone(name).bbone_segments = self.bbone_segments
        ...
        if tweak:
            ...
            if next_tweak:
                ...
                if self.params.make_preserve_volume:
                    ...
                    self.make_driver(con, 'bulge', variables=[(self.obj.pose.bones[self.bones.ctrl.master], 'volume_preserve')])

                self.rig_deform_easing(i, deform, tweak, next_tweak)

            elif next_entry:
                con = self.make_constraint(deform, 'STRETCH_TO', next_entry.org, keep_axis='SWING_Y')
                if self.params.make_preserve_volume:
                    #adding driver for volume preservation
                    self.make_driver(con, 'bulge', variables=[(self.obj.pose.bones[self.bones.ctrl.master], 'volume_preserve')])
